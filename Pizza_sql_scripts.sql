...
SELECT 
    category, COUNT(pizza_types.name) AS No_of_Pizzas
FROM
    pizza_types
GROUP BY category;
...
SELECT 
    ROUND(AVG(Quantity_Ordered), 0) as Average_per_day
FROM
    (SELECT 
...
select order_date, sum(Rev) over (order by order_date) as cum_revenue
from
(SELECT orders.order_date,
round(sum(pizzas.price * order_details.quantity),0) as Rev
FROM pizzas JOIN
order_details ON order_details.pizza_id = pizzas.pizza_id
join orders
